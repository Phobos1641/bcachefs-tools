...
#define lockdep_set_notrack_class(_lock) lockdep_set_novalidate_class(_lock)
#endif
...
static inline void *__bch2_genradix_ptr_alloc(struct __genradix *radix, size_t offset,
...
#define genradix_ptr_alloc_inlined(_radix, _idx, _gfp)			\
	(__genradix_cast(_radix)					\
	 (__genradix_ptr_inlined(&(_radix)->tree,			\
			__genradix_idx_to_offset(_radix, _idx)) ?:	\
	  __bch2_genradix_ptr_alloc(&(_radix)->tree,				\
			__genradix_idx_to_offset(_radix, _idx),		\
			NULL, _gfp)))
